...
final case class ServerImpl(
  ...
  private val healthEndpoint: ZServerEndpoint[Any, Any] =
    endpoint.get.in("tail" / "health").zServerLogic(_ => ZIO.unit)
  private val kTailSocket: ZServerEndpoint[Any, ZioStreams & WebSockets] =
    endpoint.get
      ...
      .zServerLogic(topic => tailSocketLogic(topic))
...
object ServerImpl {
  val live: URLayer[TailConfig & Broadcast, Server] =
    ZLayer {
      for {
        port <- ZIO.serviceWith[TailConfig](_.port)
