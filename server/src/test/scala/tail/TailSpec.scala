...
import tail.TestTailConfig.{Topic1, Topic2, Topic3}
import sttp.client3.*
...
object TailSpec extends ZIOSpecDefault {
  ...
  private def subscribe(topic: String): RIO[TailConfig & SttpClient, WebSocket[Task]] =
    for {
      ...
      ws <-
        sttp
          .send(
            basicRequest
              .get(uri"ws://localhost:${config.port}/tail/$topic")
